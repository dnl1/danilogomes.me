...
type MusicPlatformsProps = {
  title: string;
  description: string;
  platforms: MusicPlatformItem[];
  className?: string;
};

export function MusicPlatforms({ title, description, platforms, className = "" }: MusicPlatformsProps) {
  return (
        ...
        {platforms.map((platform) => {
          ...
          return (
            <a
              ...
              className="group flex items-center gap-3 rounded-xl border border-line/80 bg-black/20 px-4 py-3 text-sm text-fg transition duration-200 hover:scale-[1.02] hover:border-brand/80 hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
            >
